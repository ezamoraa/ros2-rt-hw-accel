...
: rclcpp::Node("HarrisNodeFPGA", options)
{
  ...
  char* fileBuf = read_binary_file(
        "/lib/firmware/xilinx/rt_hw_accel_demo/fpga_graph.xclbin",
        fileBufSize);
...
void HarrisNodeFPGA::harrisImage_fpga(const cv::Mat& in_img, cv::Mat& harris_img) const
{
  ...
  size_t image_in_size_bytes = HEIGHT * WIDTH * sizeof(unsigned char);
  ...
  OCL_CHECK(err,
            queue_->enqueueWriteBuffer(imageToDevice,       // buffer on the FPGA
                                       ...
                                       img_gray.data,         // Pointer to the data to copy
                                       nullptr, &event));

  // Set the kernel arguments
  krnl_->setArg(0, imageToDevice);
  krnl_->setArg(1, imageFromDevice);
  krnl_->setArg(2, (int)HEIGHT);
  krnl_->setArg(3, (int)in_img.cols);
  krnl_->setArg(4, (int)Thresh);
  krnl_->setArg(5, (int)k);
...
void HarrisNodeFPGA::imageCb(sensor_msgs::msg::Image::ConstSharedPtr image_msg)
{
  TRACEPOINT(
...
RCLCPP_COMPONENTS_REGISTER_NODE(rt_hw_accel_demo::HarrisNodeFPGA)
